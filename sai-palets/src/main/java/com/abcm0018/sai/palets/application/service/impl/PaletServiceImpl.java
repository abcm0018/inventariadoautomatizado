...
import com.abcm0018.sai.palets.domain.repository.PalletScanRepository;
...
public class PaletServiceImpl implements PaletService {
	...
	private final PalletScanRepository palletScanRepository;
	...
	public PaletDetailResponseDTO createPalet(CreatePaletRequestDTO request) {
		try {
			...
			validateDates(request.getPackagingDateTime(), request.getProductUseByDate());
			...
			// 5. Registrar el evento de escaneo manual
			PalletScan scan = PalletScan.builder()
					.pallet(saved)
					.station(null)
					.workshift(workshift)
					.operator(workshift.getUser())
					.scannedAt(LocalDateTime.now())
					.scanQuality(ScanQuality.MANUAL)
					.build();
			palletScanRepository.save(scan);

			log.info("Palet creado exitosamente - ID: {}, SSCC: {}, Producto: {}", saved.getId(), saved.getSscc(), packLevel.getProduct().getName());

			return paletMapper.toResponseDetail(paletRepository.findWithDetailsById(saved.getId()).orElse(saved));
	...
	public PaletDetailResponseDTO updatePalet(Long id, UpdatePaletRequestDTO request) {
		...
		if (request.getPackagingDateTime() != null && request.getProductUseByDate() != null) {
			validateDates(request.getPackagingDateTime(), request.getProductUseByDate());
		}
	...
	public Map<String, Object> getFullTraceability(String sscc) {
		...
		traceInfo.put("packagingDateTime", palet.getPackagingDateTime());
	...
	@Override
	@Transactional
	public void procesarNuevaLecturaPalet(PaletLecturaMessageDTO message, ScanQuality scanQuality) {

		log.info("LECTURA DE PALET RECIBIDA [SSCC: {}]", message.getSscc());

		// 1. Validar y enriquecer (SSCC único, ProductPackLevel por EAN, Workshift activo)
		...
		log.info("Palet persistido [SSCC: {}]", message.getSscc());

		// 3. Cargar estación (puede ser null si el stationId no existe en BD)
		ScanStation station = scanStationRepository
				.findByStationCode(message.getStationId())
				.orElse(null);

		// 4. Registrar el evento de escaneo con calidad y contexto completos
		Workshift workshift = context.getWorkshift();
		PalletScan scan = PalletScan.builder()
				.pallet(savedPalet)
				.station(station)
				.workshift(workshift)
				.operator(workshift != null ? workshift.getUser() : null)
				.scannedAt(message.getScanDate())
	...
	private void validateDates(LocalDateTime packagingDateTime, LocalDate expiryDate) {
		if (packagingDateTime != null && expiryDate != null) {
			if (packagingDateTime.toLocalDate().isAfter(expiryDate)) {
